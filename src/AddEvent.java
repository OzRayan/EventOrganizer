...
/**
 * AddEvent class extends JPanel..
 * This class prepares a JPanel for the main frame to create an event.
 *
 * Place:     University of Bedfordshire
 * Author:    Oszkar Feher
 * Date:      7/4/20
 */
class AddEvent extends JPanel {
    // Input field labels
    ...
    public AddEvent(int width, int height) {
        ...
        // Date Model. For creating an event it's enough one date model
        dateModel = new SpinnerDateModel();

        // Spinners and formatter for date and time
        dateSpinner = new JSpinner(dateModel);
        dateSpinner.setBackground(Color.lightGray);
        dateSpinner.setForeground(sv.purple);
        dateSpinner.setBorder(null);
        dateSpinner.setFont(sv.idTextFont);
        ...
        // Buttons
        create = new CreateButton("Create", sv.lightPurple, Color.lightGray, sv.buttonFont);
        create.setBounds(this.width / 2, 15, 150, 30);

        back = new CreateButton("Back", sv.lightPurple, Color.lightGray, sv.buttonFont);
        back.setBounds(this.width / 2,35 + create.getHeight(), 150, 30);
